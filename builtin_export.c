...
int ft_similar(t_data *data, int i_arg)
{
    ...
    while(data->envp[i])
    {
        if (ft_strncmp(data->tab_cmd[0].args[i_arg], data->envp[i], ft_count_egale(data->envp[i])) == 0)
        {
            data->envp[i] = ft_realloc(data->envp[i], ft_strlen(data->tab_cmd[0].args[i_arg]) + 1);
